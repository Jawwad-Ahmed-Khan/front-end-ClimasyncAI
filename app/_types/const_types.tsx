// export const Text_Styles = {
//   Heading_1: "text-3xl md:text-5xl font-bold text-center text-gray-900 mb-12 md:mb-20",
//   paragraph: "text-sm md:text-base font-medium text-gray-600 max-w-[120px] md:max-w-none leading-tight",
//   Heading_2: "text-2xl md:text-3xl font-extrabold",
// };

// export const DIV_Styles = {
//   Container: "w-full bg-white px-4",
//   Section_Padding: "py-12 md:py-24",
//   Section_Bottom_Only: "pb-12 md:pb-24",
// };

// export const Layout = {
//   maxWidth: "max-w-7xl mx-auto",
//   // Modified Grid to start at 1 column for mobile (xs), then 2 (sm), then 3 (md), then 5 (lg)
//   Grid: "grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-x-6 gap-y-12 justify-items-center",
// };

export const Text_Styles = {
  Heading_1: "text-3xl md:text-5xl font-bold text-center text-gray-900 mb-12 md:mb-20",
  paragraph: "text-sm md:text-base font-medium text-gray-600 leading-relaxed", // Removed max-w restriction for readability
  Heading_2: "text-2xl md:text-3xl font-extrabold",
};

export const DIV_Styles = {
  Container: "w-full bg-white px-4",
  Section_Padding: "py-12 md:py-24",
  Section_Bottom_Only: "pb-12 md:pb-24",
};

export const Layout = {
  maxWidth: "max-w-7xl mx-auto",
  Grid: "grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-x-6 gap-y-12 justify-items-center",
};