...
const PROJECT_DATA: ProjectItem[] = [
  {
    id: 1,
    image: "/images/home/why_this_1.webp",
    problem: {
  ...
  {
    id: 2,
    image: "/images/home/why_this_2.webp",
    problem: {
  ...
  {
    id: 3,
    image: "/images/home/why_this_3.webp",
    problem: {
        ...
        <div className="relative w-full bg-white rounded-3xl shadow-xl border border-slate-100 overflow-hidden h-[650px] md:h-[600px]">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 h-full">
                    ...
                    <div className="relative h-64 md:h-full w-full overflow-hidden bg-slate-200">
                      ...
                      {/* ------------------------------------------------------------
                         ...
                         ------------------------------------------------------------ */}
                      <Propwhy src={item.image} alt={item.solution.title} index={index} />
                      {/* ------------------------------------------------------------ */}
          ...
          <div className="hidden md:flex flex-col absolute right-6 top-1/2 -translate-y-1/2 gap-4 z-10">
            ...
            <button
              ...
              className="p-3 rounded-full bg-white shadow-lg border border-slate-100 text-slate-600 hover:text-emerald-600 hover:scale-110 transition-all disabled:opacity-50 disabled:cursor-not-allowed cursor-pointer"
            ...
            <div className="flex flex-col gap-2 items-center py-2 bg-white/80 backdrop-blur-sm rounded-full px-1 shadow-sm border border-slate-100">
              {PROJECT_DATA.map((_, idx) => (
                <button
                  ...
                  className={`w-2 rounded-full transition-all duration-500 ${idx === selectedIndex
                    ? "h-8 bg-emerald-500 shadow-md shadow-emerald-200"
                    : "h-2 bg-slate-300 hover:bg-emerald-300 cursor-pointer"
                    }`}
          ...
          <div className="flex md:hidden absolute bottom-4 left-0 right-0 justify-center gap-2 z-20 pointer-events-none">
            {PROJECT_DATA.map((_, idx) => (
              <button
                key={idx}
                onClick={() => scrollTo(idx)}
                className={`h-2 rounded-full transition-all duration-300 shadow-sm pointer-events-auto ${idx === selectedIndex ? "w-8 bg-emerald-500" : "w-2 bg-slate-300/80 hover:bg-emerald-300 cursor-pointer"
                  }`}
                aria-label={`Go to slide ${idx + 1}`}
              />
            ))}
